...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.external.receiveMessage((message) => {
    ...
    switch (parsedMessage.type) {
      ...
      case "sequence-status-update":
        ...
        break;
      case "report-availability-update":
        Alpine.store("header").reportAvailable = parsedMessage.available;
        break;
      case "step-update":
        updateStepStatus(parsedMessage.step);
        break;
      case "modal-ui-update":
        if (parsedMessage.html != null) {
          document.getElementById("modal-ui").innerHTML = parsedMessage.html;
          document
            .getElementById("modal-ui-container")
            .classList.remove("hidden");
        } else {
          document.getElementById("modal-ui-container").classList.add("hidden");
          document.getElementById("modal-ui").innerHTML = "";
        }
  ...
  function updateSequenceDialog(message) {
    ...
    if (firstSequence) {
      ...
      emptyState.textContent =
        "No sequences available for the configured sequence types.";
  ...
  openReportButton.addEventListener("click", () => {
    sendMessage({ command: "open-report" });
  });
});
