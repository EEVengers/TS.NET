...
namespace TS.NET
{
    ...
    public sealed unsafe class NativeMemoryAligned_Old<T> : IDisposable
        where T : unmanaged
    {
        public static readonly NativeMemoryAligned_Old<T> Empty;
        ...
        static NativeMemoryAligned_Old()
        {
            Empty = new NativeMemoryAligned_Old<T>(0);
            Empty.Dispose();
        }

        public NativeMemoryAligned_Old(long length, uint alignment = 4096, bool addMemoryPressure = false)
        {
            this.length = length;
