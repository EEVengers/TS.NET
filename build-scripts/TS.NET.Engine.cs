...
// TS.NET.Engine cross-platform build script using .NET script capabilities.
...
var projectFolder = Path.Combine(rootDirectory, "source", "TS.NET.Engine");
var csprojPath = Path.Combine(projectFolder, "TS.NET.Engine.csproj");
...
var buildRoot = Path.Combine(rootDirectory, "build", "TS.NET.Engine");
...
static IReadOnlyList<string> ResolveTargetRids(string[] args)
{
	...
	{
		Console.ForegroundColor = ConsoleColor.Red;
		Console.WriteLine("Usage: dotnet TS.NET.Engine.cs [-rid <win-x64|linux-x64|osx-x64|osx-arm64|all>]");
		Console.ResetColor();
